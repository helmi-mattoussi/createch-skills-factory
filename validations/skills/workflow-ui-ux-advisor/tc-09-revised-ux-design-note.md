...
- no in-scope UX behavior changed
- the revision explicitly preserves `restore` as out of scope because no validated framing or PD supports that expansion
...
- D1: the revision request asks for restore behavior, but no validated framing or PD supports adding restore to this UX note
